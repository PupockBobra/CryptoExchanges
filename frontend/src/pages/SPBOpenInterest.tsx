...
interface SpbOiRow {
  ...
  oi_contracts: number   // open positions in contracts
  oi_rub:       number   // open-position notional, RUB
}
...
interface ChartProps { ticker: string; name: string; rows: SpbOiRow[] }

function SpbOiChart({ ticker, name, rows }: ChartProps) {
  ...
  useEffect(() => {
    ...
    // Open-interest notional runs tens-to-hundreds of millions of roubles → ₽M.
    const rubByDate = new Map<string, number>()
    const conByDate = new Map<string, number>()
    rows.forEach(r => { rubByDate.set(r.date, r.oi_rub); conByDate.set(r.date, r.oi_contracts) })
    const y = dates.map(d => { const v = rubByDate.get(d); return v != null ? v / 1e6 : null })
    const contracts = dates.map(d => conByDate.get(d) ?? null)

    const trace: Plotly.Data = {
      type: 'bar', name, x: labels, y,
      customdata: contracts,
      marker: { color: SPB_COLOR, opacity: 0.85 },
      hovertemplate: '₽%{y:.1f}M · %{customdata:,} contracts<extra></extra>',
    }
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: 4 }}>
          <button
            onClick={() => exportDailyCsv(
              rows.map(r => ({ date: r.date, symbol: name, exchange: 'spb', volume_rub: r.oi_rub })),
              name,
              `spb-oi-${ticker}.csv`,
            )}
            style={{
...
export function SPBOpenInterest() {
  const [allRows, setAllRows] = useState<SpbOiRow[]>([])
  ...
  const load = async () => {
    setLoading(true)
    try {
      const data = await fetchJson<SpbOiRow[]>(`${API}/open-interest`)
      setAllRows(data)
      setLastSync(new Date())
    } catch (e) {
      console.error('SPBOpenInterest: failed to load open interest', e)
    } finally {
      setLoading(false)
    }
  }
  ...
  const rowsByTicker = useMemo(() => {
    const map = new Map<string, SpbOiRow[]>()
    for (const r of allRows) {
  ...
  return (
    <div>
      <div className="page-toolbar">
        <h1>SPB Open Interest</h1>
        <div style={{ fontSize: 12, color: 'var(--muted)', marginLeft: 'auto' }}>
          СПБ Биржа perpetual futures · daily open interest · RUB · last 30 days
      ...
      <div className="card" style={{ marginBottom: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
        <div style={{ padding: '12px 20px 12px 16px', borderRight: '1px solid var(--border)' }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--text)', lineHeight: 1.5 }}>
            Открытый интерес (на конец дня) по вечным фьючерсам СПБ Биржи за последние 30 дней, по каждому инструменту.
        ...
        <div style={{ padding: '12px 16px 12px 20px' }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--text)', lineHeight: 1.5 }}>
            ₽M (номинал открытых позиций в миллионах рублей). В подсказке — число контрактов.
          </div>
        </div>
      </div>

      {loading ? (
        <p className="empty">Loading SPB open-interest data…</p>
      ) : tickers.length === 0 ? (
